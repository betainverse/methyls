...
noise = 4000
yscalefactor = 100000
exclude = [12,18,19,29,40,41,51,61,86,92,134,135,137,141]
maybe_exclude = [6,7,10,15,17,20,73,125]


# Sample Name for titles
sample_name = '20140911 Zn AdcR T1'
...
rhTable = '/Users/edmonds/Documents/GiedrocLab/Group Mtg 5/AdcR_relaxation/20140911_ZnAdcR_T1.txt'
...
def T2barplot(rhDF):
    delays=rhDF.columns.values
    assignments = [x for x in rhDF[delays[0]].keys() if int(x[1:-3]) not in exclude]
    intassignments = [int(x[1:-3]) for x in rhDF[delays[0]].keys() if int(x[1:-3]) not in exclude]
...
def plotcurve(assignment,rhDF):
    ...
    T2expression=r'$T2 = %.1f\pm%.2f$ s'%(T2,T2err)
...
def plot1curve(assignment,rhDF,ax):
    ...
    #T2ms = T2*1000
    #dTms = dT*1000
    T2expression = r'$%.2f\pm%.2f  s$'%(T2,dT)
    Assexpression = '%s: T1 ='%assignment[0:-3]
    xvalues = interpolate(delays)
    ...
    ax.annotate(Assexpression+'\n'+T2expression,xy=(5,-10),xycoords='axes points',
                horizontalalignment='left',verticalalignment='top')
    print "%s\t%0.8f\t%0.8f"%(assignment[1:-3], T2, dT)
